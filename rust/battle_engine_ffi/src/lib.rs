...
fn process_battle_rounds(input: BattleInput) -> BattleOutput {
    ...
    for _ in 0..6 {
        ...
        process_combat(
            ...
            &mut defender_units,
            &mut round,
            &attacker_units_metadata,
            &defender_units_metadata,
            true,
        );
        process_combat(
            &mut defender_units,
        ...
        calculate_fleet_results(
            &mut round,
            &attacker_units,
            &defender_units,
            &attacker_fleet_metadata,
            &defender_fleet_metadata,
            &attacker_fleet_owners,
            &defender_fleet_owners,
        );
...
fn compress_fleet_results(
    ...
    HashMap<i16, BattleUnitCount>,
    HashMap<i16, BattleUnitCount>,
    HashMap<i16, BattleUnitCount>,
) {
    // Filter units by fleet
    let fleet_units: Vec<&BattleUnitInstance> = units
        .iter()
    ...
    for attacker in attackers.iter() {
        ...
        while continue_attacking {
            ...
            if target.current_hull_plating / target_metadata.hull_plating < 0.7 {
                let explosion_chance =
                    100.0 - ((target.current_hull_plating / target_metadata.hull_plating) * 100.0);
            ...
            continue_attacking =
                if let Some(rapidfire_amount) = attacker_metadata.rapidfire.get(&target.unit_id) {
...
fn cleanup_round(
    ...
    attackers.retain(|unit| {
        ...
        if unit.current_hull_plating <= 0.0 {
            increment_battle_unit_count_amount(
...
fn calculate_fleet_results(
    ...
    for (&fleet_mission_id, initial_units) in attacker_fleets {
        ...
        round.attacker_fleet_results.insert(
            fleet_mission_id,
            AttackerFleetResult {
                fleet_mission_id,
                owner_id,
    ...
    for (&fleet_mission_id, initial_units) in defender_fleets {
        ...
        round.defender_fleet_results.insert(
            fleet_mission_id,
            DefenderFleetResult {
                fleet_mission_id,
                owner_id,
...
fn increment_battle_unit_count_amount(
    hash_map: &mut HashMap<i16, BattleUnitCount>,
    unit_id: i16,
    amount_to_increment: u32,
) {
    let count = hash_map
        .entry(unit_id)
        .or_insert(BattleUnitCount { unit_id, amount: 0 });
